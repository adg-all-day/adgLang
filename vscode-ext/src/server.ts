...
console.log("[Server] ADGLANG Language Server initializing...");
...
connection.onDidChangeConfiguration((change) => {
  if (hasConfigurationCapability) {
    ...
    globalSettings = (change.settings.adgLangLanguageServer ||
      defaultSettings) as ExampleSettings;

    // Update symbol index with ADGLANG_HOME
    if (globalSettings.adgLangHome) {
      symbolIndex.setBplHome(globalSettings.adgLangHome);
    }
  }
...
function findWorkspaceLibDir(startDir: string): string | null {
  // Check ADGLANG_HOME environment variable first
  if (process.env.ADGLANG_HOME) {
    const adgLangHomeLib = path.join(process.env.ADGLANG_HOME, "lib");
    if (fs.existsSync(adgLangHomeLib)) {
      return adgLangHomeLib;
    }
    // Also check if ADGLANG_HOME itself is the lib dir (some users might set it that way)
    if (fs.existsSync(path.join(process.env.ADGLANG_HOME, "string.adg"))) {
      return process.env.ADGLANG_HOME;
    }
  }
...
function resolveImportToFile(
  ...
  if (importPath.startsWith("std/") || importPath.startsWith("std\\")) {
    ...
    if (!candidate.endsWith(".adg")) candidate += ".adg";
    return fs.existsSync(candidate) ? candidate : null;
  }

  let resolvedPath = path.resolve(currentDir, importPath || "");
  if (!fs.existsSync(resolvedPath) && fs.existsSync(resolvedPath + ".adg")) {
    resolvedPath += ".adg";
  }
...
async function validateTextDocument(textDocument: TextDocument): Promise<void> {
  ...
  if (settings?.adgLangHome) {
    process.env.ADGLANG_HOME = settings.adgLangHome;
    symbolIndex.setBplHome(settings.adgLangHome);
  } else {
    // Auto-detect ADGLANG_HOME if not set
    const libDir = findWorkspaceLibDir(currentDir);
    if (libDir) {
      process.env.ADGLANG_HOME = path.dirname(libDir);
...
connection.onCompletionResolve((item: CompletionItem): CompletionItem => {
  if (item.kind === CompletionItemKind.Keyword) {
    item.detail = "Keyword";
    item.documentation = `ADGLANG keyword: ${item.label}`;
  } else if (item.kind === CompletionItemKind.Class) {
    item.detail = "Type";
    item.documentation = `ADGLANG built-in type: ${item.label}`;
  }
  return item;
});
