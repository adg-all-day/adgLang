/**
 * Comprehensive tests for todo_app/main.adg
...
describe("Todo App - Completions", () => {
  ...
  it("should complete imported bare functions (sprintf, strcpy, etc.)", () => {
    ...
    console.log("[Test] Indexing todo_app/main.adg...");
  ...
  it("should complete members after App.", () => {
    // Index adgLang-express so App type can be resolved
    symbolIndex.indexFile(TODO_APP_PATH);
  ...
  it("should complete members after req. (Request object)", () => {
    // Index adgLang-express so Request type can be resolved
    symbolIndex.indexFile(TODO_APP_PATH);

    // Test "req." completion in function parameter context
    const testContent = `import [Request] from "adgLang-express";\nframe test(req: *Request) {\n    local y = req.x;\n}`;
    const testPath = path.resolve(
      __dirname,
      "../../../tmp/test-req-completion.adg",
    );
    const testDoc = TextDocument.create(
      `file://${testPath}`,
      "adgLang",
  ...
  it("should complete members after res. (Response object)", () => {
    // Index adgLang-express so Response type can be resolved
    symbolIndex.indexFile(TODO_APP_PATH);

    const testContent = `import [Response] from "adgLang-express";\nframe test(res: *Response) {\n    local z = res.x;\n}`;
    const testPath = path.resolve(
      __dirname,
      "../../../tmp/test-res-completion.adg",
    );
    const testDoc = TextDocument.create(
      `file://${testPath}`,
      "adgLang",
  ...
  it("should complete Option enum variants", () => {
    const testContent = `import [Option] from "std/option.adg";\nframe test() {\n    local x = Option.\n}`;
    const testDoc = TextDocument.create(
      `file://${TODO_APP_PATH}`,
      "adgLang",
      1,
...
describe("Todo App - Edge Cases", () => {
  ...
  it("should handle completion inside loop blocks", () => {
    const testContent = `frame test() {\n    local i: int = 0;\n    loop (i < 10) {\n        \n        i = i + 1;\n    }\n}`;
    const testPath = path.resolve(__dirname, "../../../tmp/test-loop-vars.adg");
    const testDoc = TextDocument.create(
      `file://${testPath}`,
      "adgLang",
      1,
