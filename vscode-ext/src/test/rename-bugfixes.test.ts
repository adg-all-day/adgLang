...
`;
    ...
    const filePath = path.join(TMP_DIR, "test-param-type.adg");
    fs.writeFileSync(filePath, code);

    const doc = TextDocument.create(`file://${filePath}`, "adgLang", 1, code);
...
`;
    ...
    const filePath = path.join(TMP_DIR, "test-shadow.adg");
    fs.writeFileSync(filePath, code);
...
`;
    ...
    const filePath = path.join(TMP_DIR, "test-param-body.adg");
    fs.writeFileSync(filePath, code);
