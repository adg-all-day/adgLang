...
describe("Bare Import Completion", () => {
  it("should complete bare imported functions like strcpy", () => {
    ...
    const testContent = `import sprintf, printf, strcpy, strlen from "adgLang-express";\n\nframe test() {\n    \n}`;

    const testDoc = TextDocument.create(
      `file://${path.resolve(__dirname, "../../../tmp/test.adg")}`,
      "adgLang",
      1,
      testContent,
    );
    ...
    const completions = completionHandler.handle(
      {
        textDocument: {
          uri: `file://${path.resolve(__dirname, "../../../tmp/test.adg")}`,
        },
