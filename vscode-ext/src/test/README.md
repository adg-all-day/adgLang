# adgLang VS Code Extension - Test Suite

This directory holds automated tests for adgLang language server features.
...
Contains adgLang test files used as test data:

- **completion-test.adg**: Includes different scenarios for testing autocompletion features

### `languageServer.test.ts`

Main test suite for language server features:

- **Completion tests**: Verify completions for struct members, local variables, and nested scopes
- **Hover tests**: (To be added) Test hover information
- **Definition tests**: (To be added) Test go-to-definition functionality

## Test Framework

The suite uses **Bun's built-in test runner** (`bun:test`), which gives you:
...
1. **Create or update fixture files** in `fixtures/` with valid adgLang code
...
- Test fixtures must contain **valid adgLang syntax**
...
2. **Imported Function Resolution**: Functions imported from packages (like `sprintf`, `printf` from "adgLang-express") resolve correctly
3. **Nested Scope Support**: Variables declared in loops are accessible and provide completions
4. **Robust Error Handling**: Invalid positions do not crash the language server
