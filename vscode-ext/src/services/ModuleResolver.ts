 ...
 * - Local files (./file.adg, ../file.adg)
 * - Standard library (std/*)
 * - Local packages (adgLang_modules/)
 * - Global packages (~/.adg/packages/)
 */
...
export class ModuleResolver {
  private adgLangHome: string | null = null;
  private cache = new Map<string, ResolvedModule | null>();

  constructor(adgLangHome?: string) {
    this.adgLangHome = adgLangHome || process.env.ADGLANG_HOME || null;
  }

  /**
   * Update ADGLANG_HOME path
   */
  setBplHome(adgLangHome: string | null): void {
    this.adgLangHome = adgLangHome;
    this.cache.clear(); // Clear cache when ADGLANG_HOME changes
  }
  ...
  private resolveRelative(
    ...
    const candidates = [
      ...
      path.resolve(currentDir, `${importPath}.adg`),
    ];
  ...
  private resolveAbsolute(importPath: string): ResolvedModule | null {
    const candidates = [importPath, `${importPath}.adg`];
  ...
  /**
   * Resolve import from local package (adgLang_modules/)
   */
  private resolveLocalPackage(
    importPath: string,
    currentDir: string,
  ): ResolvedModule | null {
    // Walk up directory tree to find adgLang_modules/
    let dir = currentDir;
    const maxDepth = 10;

    for (let i = 0; i < maxDepth; i++) {
      const adgLangModulesDir = path.join(dir, "adgLang_modules");
      if (fs.existsSync(adgLangModulesDir)) {
        ...
        const packageDir = path.join(adgLangModulesDir, packageName);
  ...
  /**
   * Resolve import from global package (~/.adg/packages/)
   */
  private resolveGlobalPackage(importPath: string): ResolvedModule | null {
    const globalPackagesDir = path.join(os.homedir(), ".adg", "packages");
  ...
  private resolveWithinPackage(
    ...
    let mainEntry = "index.adg"; // Default to index.adg
    ...
    if (!subPath) {
      const entryPoints = [mainEntry, "index.adg", "main.adg"];
    ...
    const candidates = [
      ...
      path.join(packageDir, `${subPath}.adg`),
      path.join(packageDir, subPath, "index.adg"),
    ];
   ...
   * 1. ADGLANG_HOME/lib (if ADGLANG_HOME is set)
   * 2. Walk up from current directory to find lib/ with string.adg
   */
  private findLibDirectory(): string | null {
    // 1. Try ADGLANG_HOME
    if (this.adgLangHome) {
      const candidates = [
        path.join(this.adgLangHome, "lib"),
        this.adgLangHome, // ADGLANG_HOME might be lib itself
      ];

      for (const candidate of candidates) {
        if (
          fs.existsSync(candidate) &&
          fs.existsSync(path.join(candidate, "string.adg"))
        ) {
          return candidate;
        }
      }
    ...
    for (let i = 0; i < maxDepth; i++) {
      ...
      if (
        ...
        fs.existsSync(path.join(libDir, "string.adg"))
      ) {
        return libDir;
      }
