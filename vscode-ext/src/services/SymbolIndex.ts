...
export class SymbolIndex {
  ...
  constructor(adgLangHome?: string) {
    this.resolver = new ModuleResolver(adgLangHome);
    this.currentBplHome = adgLangHome || null;
  }

  /**
   * Update ADGLANG_HOME and clear caches only if it changed
   */
  setBplHome(adgLangHome: string | null): void {
    if (this.currentBplHome === adgLangHome) {
      return; // No change, don't clear
    }
    this.currentBplHome = adgLangHome;
    this.resolver.setBplHome(adgLangHome);
    this.clear();
  }
        ...
        for (const stmt of program.statements) {
          if (stmt.kind === "Import") {
            ...
            if (resolved) {
              ...
              // Index bare function imports (e.g., import sprintf, printf from "adgLang-express")
  ...
  private getModuleInfo(
    ...
    if (
      filePath.includes("/adgLang_modules/") ||
      filePath.includes("\\adgLang_modules\\")
    ) {
      // Extract package name
      const match = filePath.match(/adgLang_modules[\/\\]([^\/\\]+)/);
      return {
    ...
    if (
      filePath.includes("/.adg/packages/") ||
      filePath.includes("\\.adg\\packages\\")
    ) {
      // Extract package name
