// Tutorial JavaScript - ADGLANG Learn from Zero to Hero
...
function loadProgress() {
  const saved = localStorage.getItem("adgLang-tutorial-progress");
  if (saved) {
  ...
  return `
    ...
    <div class="section-body">
      <div class="code-section">
        ...
        <pre class="code-block">${highlightADGLANG(section.code)}</pre>
      </div>
      ${expectedOutput}
      ${
        lineExplanations
      ...
      ${
        ...
        <div class="solution-content" id="solution-${index}">
          <div class="code-section">
            ...
            <pre class="code-block">${highlightADGLANG(section.solution)}</pre>
          </div>
        </div>
      `
          : ""
...
function openCodeModal(code) {
  ...
  require(["vs/editor/editor.main"], function () {
    // Register ADGLANG language if not already
    if (!monaco.languages.getLanguages().find((l) => l.id === "adgLang")) {
      monaco.languages.register({ id: "adgLang" });
      monaco.languages.setMonarchTokensProvider(
        "adgLang",
        getADGLANGLanguageDefinition(),
      );
    }
...
// Simple ADGLANG syntax highlighter
function highlightADGLANG(code) {
  // First escape HTML
...
// Get ADGLANG language definition for Monaco
function getADGLANGLanguageDefinition() {
  return {
    keywords: [
      "frame",
      "ret",
