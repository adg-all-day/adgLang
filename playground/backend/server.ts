...
async function compileAndRun(req: CompileRequest): Promise<CompileResponse> {
  ...
  const tempDir = path.join("/tmp", `adgLang-playground-${Date.now()}`);
  fs.mkdirSync(tempDir, { recursive: true });

  const sourceFile = path.join(tempDir, "main.adg");
  ...
  try {
    ...
    try {
      ...
      const runtimeLLPath = path.join(adgLangHome, "lib", "runtime.ll");
      ...
      const runtimeSupportPath = path.join(adgLangHome, "lib", "runtime_support.o");
  ...
  async fetch(req) {
    ...
    if (url.pathname === "/format" && req.method === "POST") {
      try {
        ...
        // Parse code first
        const parser = new Parser(body.code, "temp.adg");
...
logger.info("🚀 ADGLANG Playground Server Started");
logger.info("=".repeat(60));
