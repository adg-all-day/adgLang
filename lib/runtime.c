 ...
 * Then link with ADGLANG programs
 */
...
/* ============ Constants ============ */
#define ADGLANG_MAX_STACK_DEPTH 10000
#define ADGLANG_MAX_BACKTRACE_FRAMES 64
#define ADGLANG_STACK_FRAME_BUFFER_SIZE 256
...
__attribute__((noreturn)) void __adgLang_throw_null_access(const char *func_name, const char *expr_str, int32_t line, int32_t col);
__attribute__((noreturn)) void __adgLang_throw_stack_overflow(void);
__attribute__((noreturn)) void __adgLang_throw_division_by_zero(const char *func_name, int32_t line, int32_t col);
__attribute__((noreturn)) void __adgLang_throw_index_out_of_bounds(int32_t index, int32_t size, const char *func_name, int32_t line, int32_t col);
void __adgLang_unwind_to_defer_node(DeferNode *target);

/* ============ Global State ============ */

/* Stack depth counter for overflow detection */
int32_t __adgLang_stack_depth = 0;

/* Argc/Argv storage */
int32_t __adgLang_argc_value = 0;
char **__adgLang_argv_value = NULL;

/* Stack frame names for better traces (circular buffer) */
static const char *__adgLang_frame_names[ADGLANG_MAX_STACK_DEPTH];
static const char *__adgLang_frame_files[ADGLANG_MAX_STACK_DEPTH];
static int32_t __adgLang_frame_lines[ADGLANG_MAX_STACK_DEPTH];
...
static void print_stack_trace(void) {
    ...
    void *buffer[ADGLANG_MAX_BACKTRACE_FRAMES];
    int nptrs = backtrace(buffer, ADGLANG_MAX_BACKTRACE_FRAMES);
...
/**
 * Print ADGLANG-level stack frames if available
 */
static void print_adgLang_stack_trace(void) {
    if (__adgLang_stack_depth <= 0) {
        return;
    }
    
    fprintf(stderr, "\n%s=== ADGLANG Call Stack ===%s\n", COLOR_CYAN, COLOR_RESET);
    
    int depth = __adgLang_stack_depth;
    if (depth > ADGLANG_MAX_STACK_DEPTH) depth = ADGLANG_MAX_STACK_DEPTH;
    
    for (int i = depth - 1; i >= 0 && i >= depth - 20; i--) {
        const char *name = __adgLang_frame_names[i];
        const char *file = __adgLang_frame_files[i];
        int32_t line = __adgLang_frame_lines[i];
...
static void __adgLang_install_signal_handlers(void) {
    struct sigaction sa;
...
void __adgLang_enter_stack_frame(void) {
    __adgLang_stack_depth++;
    
    if (__adgLang_stack_depth > ADGLANG_MAX_STACK_DEPTH) {
        __adgLang_throw_stack_overflow();
    }
}

void __adgLang_enter_stack_frame_named(const char *name, const char *file, int32_t line) {
    int idx = __adgLang_stack_depth;
    __adgLang_stack_depth++;
    
    if (idx < ADGLANG_MAX_STACK_DEPTH) {
        __adgLang_frame_names[idx] = name;
        __adgLang_frame_files[idx] = file;
        __adgLang_frame_lines[idx] = line;
    }
...
void __adgLang_exit_stack_frame(void) {
    if (__adgLang_stack_depth > 0) {
        __adgLang_stack_depth--;
        
        if (__adgLang_stack_depth < ADGLANG_MAX_STACK_DEPTH) {
            __adgLang_frame_names[__adgLang_stack_depth] = NULL;
            __adgLang_frame_files[__adgLang_stack_depth] = NULL;
            __adgLang_frame_lines[__adgLang_stack_depth] = 0;
        }
    }
}
...
int32_t __adgLang_argc(void) {
    return __adgLang_argc_value;
}

char *__adgLang_argv_get(int32_t index) {
    if (index < 0 || index >= __adgLang_argc_value) {
        return NULL;
    }
    return __adgLang_argv_value[index];
}

/* ============ Memory Utilities ============ */

int __adgLang_mem_is_zero(const void *ptr, size_t n) {
...
void __adgLang_check_null(void *ptr, const char *func_name, const char *expr_str, int32_t line, int32_t col) {
    if (ptr == NULL) {
        __adgLang_throw_null_access(func_name, expr_str, line, col);
    }
}
...
void __adgLang_throw_null_access(const char *func_name, const char *expr_str, int32_t line, int32_t col) {
    if (exception_top != NULL) {
        ...
        if (err) {
            ...
            /* Unwind defers */
            __adgLang_unwind_to_defer_node(exception_top->defer_top);
...
__attribute__((noreturn))
void __adgLang_throw_stack_overflow(void) {
    if (exception_top != NULL) {
    ...
    fprintf(stderr, "\n%sMaximum call stack depth (%d) exceeded%s\n", 
            COLOR_YELLOW, ADGLANG_MAX_STACK_DEPTH, COLOR_RESET);
...
__attribute__((noreturn))
void __adgLang_throw_division_by_zero(const char *func_name, int32_t line, int32_t col) {
    if (exception_top != NULL) {
...
__attribute__((noreturn))
void __adgLang_throw_index_out_of_bounds(int32_t index, int32_t size, const char *func_name, int32_t line, int32_t col) {
    if (exception_top != NULL) {
