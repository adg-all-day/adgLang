...
describe("AST Module", () => {
  it("should compile the AST module definitions", () => {
    // We create a small program that imports ast.adg to verify it compiles and links
    const srcDir = path.resolve(process.cwd(), "src");
    const astPath = path.join(srcDir, "ast.adg");
    ...
    const code = `
      ...
      # Use relative import for diagnostics or pass it from JS, but here we can just target src/diagnostics.adg
