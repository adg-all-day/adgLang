...
function getExampleDirectories(dir = EXAMPLES_DIR): string[] {
  ...
  for (const file of list) {
    ...
    if (
      ...
      !stat.isSymbolicLink() &&
      file !== "node_modules" &&
      file !== "adgLang_modules"
    ) {
      if (fs.existsSync(path.join(fullPath, "main.adg"))) {
...
describe("Integration Tests", () => {
  ...
  for (const example of examples) {
    ...
    const relativeMainFile = path.relative(
      process.cwd(),
      path.join(exampleDir, "main.adg"),
    );
    const configFile = path.join(exampleDir, "test_config.json");

    if (
      fs.existsSync(path.join(exampleDir, "main.adg")) &&
      ...
      it(`should run example: ${example}`, () => {
        ...
        const result = spawnSync(
          ...
          [relativeMainFile, ...(config.args || [])],
          {
            env: {
              ...process.env,
              ADGLANG_HOME: process.cwd(), // Set ADGLANG_HOME to current directory for stdlib resolution
              ...(config.env || {}),
            },
            input: config.input || "",
            encoding: "utf-8",
