...
describe("Package Manager CLI", () => {
  ...
  const adgLangPath = path.join(__dirname, "..", "index.ts");

  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "adgLang-cli-test-"));
  ...
  describe("init command", () => {
    test("should create adgLang.json file", () => {
      const result = spawnSync("bun", [adgLangPath, "init"], {
        cwd: tempDir,
        encoding: "utf-8",
      });

      expect(result.status).toBe(0);
      expect(fs.existsSync(path.join(tempDir, "adgLang.json"))).toBe(true);

      const manifest = JSON.parse(fs.readFileSync("adgLang.json", "utf-8"));
      expect(manifest.name).toBeTruthy();
      expect(manifest.version).toBe("1.0.0");
    });

    test("should fail if adgLang.json already exists", () => {
      fs.writeFileSync("adgLang.json", "{}");
  ...
  describe("pack command", () => {
    test("should create package tarball", () => {
      ...
      fs.writeFileSync("adgLang.json", JSON.stringify(manifest, null, 2));
      fs.writeFileSync("index.adg", "export test;");

      const result = spawnSync("bun", [adgLangPath, "pack"], {
        cwd: tempDir,
        encoding: "utf-8",
      });
    ...
    test("should fail without adgLang.json", () => {
      const result = spawnSync("bun", [adgLangPath, "pack"], {
        cwd: tempDir,
        encoding: "utf-8",
      });
  ...
  describe("install command", () => {
    test("should install package locally", () => {
      ...
      fs.writeFileSync(
        path.join(packageDir, "adgLang.json"),
        JSON.stringify(manifest, null, 2),
      );
      fs.writeFileSync(path.join(packageDir, "index.adg"), "export test;");

      // Pack it
      const packResult = spawnSync("bun", [adgLangPath, "pack"], {
        cwd: packageDir,
        encoding: "utf-8",
      });
      ...
      const installResult = spawnSync(
        "bun",
        [adgLangPath, "install", tarballPath],
        {
          cwd: projectDir,
          encoding: "utf-8",
        },
      ...
      expect(
        fs.existsSync(path.join(projectDir, "adgLang_modules", "install-cli-test")),
      ).toBe(true);
    });
  });
  ...
  describe("list command", () => {
    test("should list installed packages", () => {
      ...
      const installResult = spawnSync(
        "bun",
        [adgLangPath, "install", "list-cli-test-1.5.0.tgz"],
        {
          cwd: tempDir,
          encoding: "utf-8",
        },
      ...
      // List packages
      const listResult = spawnSync("bun", [adgLangPath, "list"], {
        cwd: tempDir,
        encoding: "utf-8",
      });
    ...
    test("should show message when no packages installed", () => {
      const result = spawnSync("bun", [adgLangPath, "list"], {
        cwd: tempDir,
        encoding: "utf-8",
      });
  ...
  describe("uninstall command", () => {
    test("should uninstall installed package", () => {
      ...
      spawnSync("bun", [adgLangPath, "pack"], { cwd: tempDir });
      spawnSync("bun", [adgLangPath, "install", "uninstall-cli-test-1.0.0.tgz"], {
        cwd: tempDir,
      });

      // Verify it's installed
      let listResult = spawnSync("bun", [adgLangPath, "list"], {
        cwd: tempDir,
        encoding: "utf-8",
      });
      ...
      const uninstallResult = spawnSync(
        "bun",
        [adgLangPath, "uninstall", "uninstall-cli-test"],
        {
          cwd: tempDir,
          encoding: "utf-8",
        },
      ...
      // Verify it's gone
      listResult = spawnSync("bun", [adgLangPath, "list"], {
        cwd: tempDir,
        encoding: "utf-8",
      });
    ...
    test("should support remove alias", () => {
      ...
      spawnSync("bun", [adgLangPath, "pack"], { cwd: tempDir });
      spawnSync("bun", [adgLangPath, "install", "remove-cli-test-1.0.0.tgz"], {
        cwd: tempDir,
      });
      ...
      const removeResult = spawnSync(
        "bun",
        [adgLangPath, "remove", "remove-cli-test"],
        {
          cwd: tempDir,
          encoding: "utf-8",
        },
    ...
    test("should fail when uninstalling non-existent package", () => {
      const result = spawnSync(
        "bun",
        [adgLangPath, "uninstall", "non-existent-package"],
        {
          cwd: tempDir,
          encoding: "utf-8",
        },
  ...
  describe("Package import integration", () => {
    test("should compile code that imports from installed package", () => {
      ...
      fs.writeFileSync(
        path.join(packageDir, "index.adg"),
        `export add;
export multiply;
...
frame main() ret int {
    ...
    return 0;
      ...
      // Compile it
      const compileResult = spawnSync("bun", [adgLangPath, "main.adg"], {
        cwd: projectDir,
        encoding: "utf-8",
      });
