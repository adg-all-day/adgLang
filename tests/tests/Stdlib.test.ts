...
function compileAndRun(sourceCode: string) {
  ...
  try {
    ...
    if (result.status !== 0) {
      ...
      throw new Error(`ADGLANG execution failed with code ${result.status}`);
    }
  ...
  test("FS basic operations", async () => {
    const output = compileAndRun(`
      ...
      frame main() {
        ...
        # Cleanup is hard from ADGLANG without remove(), so we rely on test harness cleanup or ignore it
      }
    `);
