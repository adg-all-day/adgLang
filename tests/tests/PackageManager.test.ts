...
describe("PackageManager", () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "adgLang-test-"));
  ...
  describe("Package Initialization", () => {
    test("should create a valid adgLang.json manifest", () => {
      const manifestPath = path.join(tempDir, "adgLang.json");
  ...
  describe("Package Creation", () => {
    test("should create a package tarball", () => {
      ...
      fs.writeFileSync("index.adg", "frame test() ret int { return 42; }");
    ...
    test("should include all source files in package", () => {
      ...
      fs.writeFileSync("utils.adg", "frame helper() ret int { return 1; }");
    ...
    test("should exclude node_modules and adgLang_modules from package", () => {
      const manifest = {
        name: "exclude-test",
        version: "1.0.0",
      };
      ...
      fs.writeFileSync("adgLang.json", JSON.stringify(manifest, null, 2));
      fs.writeFileSync("index.adg", "frame test() ret int { return 0; }");

      // Create directories that should be excluded
      fs.mkdirSync("node_modules", { recursive: true });
      fs.mkdirSync("adgLang_modules", { recursive: true });
      fs.writeFileSync("node_modules/test.js", "module.exports = {};");
      fs.writeFileSync("adgLang_modules/test.adg", "frame test() {}");
  ...
  describe("Package Installation", () => {
    test("should install package locally", () => {
      ...
      expect(fs.existsSync(path.join(installedPath, "adgLang.json"))).toBe(true);
      expect(fs.existsSync(path.join(installedPath, "index.adg"))).toBe(true);
    });
  ...
  describe("Package Uninstallation", () => {
    ...
    test("should throw error for invalid package directory", () => {
      // Create a directory without adgLang.json
      const fakePackageDir = path.join(tempDir, "adgLang_modules", "fake-pkg");
      fs.mkdirSync(fakePackageDir, { recursive: true });
      fs.writeFileSync(path.join(fakePackageDir, "index.adg"), "// fake");
  ...
  describe("Package Resolution", () => {
    test("should resolve local package import", () => {
      ...
      expect(resolved).toContain("index.adg");
  ...
  describe("Package Hash Calculation", () => {
    ...
    test("should calculate different hash for different content", () => {
      ...
      // Modify content
      fs.writeFileSync("index.adg", "export modified;");
