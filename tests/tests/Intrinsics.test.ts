...
function compileAndRun(sourceCode: string) {
  const tempFile = path.join(
    __dirname,
    `temp_intrinsics_${Math.random().toString(36).substring(7)}.adg`,
  );
...
describe("Intrinsics", () => {
  ...
  test("Bit Manipulation Intrinsics", () => {
    const output = compileAndRun(`
      import [Int] from "std/primitives.adg";
      extern printf(fmt: string, ...) ret int;
  ...
  test("New Intrinsics (fma, stack, frame)", () => {
    const output = compileAndRun(`
      import fma, frameaddress, returnaddress, stacksave, stackrestore from "std/intrinsics.adg";
      extern printf(fmt: string, ...) ret int;
