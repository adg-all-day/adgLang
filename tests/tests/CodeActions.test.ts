...
const TEST_DIR = join(tmpdir(), "adgLang-code-actions-test");
...
describe("Code Action Tests - Error Suggestions", () => {
  describe("Unknown Type Errors", () => {
    it("creates test for unknown standard library type", () => {
      const testFile = join(TEST_DIR, "ca_unknown_std_type.adg");
      const code = `
frame main() ret int {
...
`;
    ...
    it("creates test for unknown type from local file", () => {
      const typesFile = join(TEST_DIR, "custom_types.adg");
      const mainFile = join(TEST_DIR, "ca_unknown_local_type.adg");
...
frame main() ret int {
    ...
    # QUICK FIX: Import CustomData from ./custom_types.adg
    return 0;
}
`,
      );
      ...
      console.log(
        "  - Suggested fix: Import CustomData from ./custom_types.adg",
      );
  ...
  describe("Unknown Symbol Errors", () => {
    it("creates test for unknown function with typo", () => {
      const testFile = join(TEST_DIR, "ca_typo.adg");
      const code = `
...
`;
    ...
    it("creates test for unknown function from import", () => {
      const utilsFile = join(TEST_DIR, "utils.adg");
      const mainFile = join(TEST_DIR, "ca_missing_import.adg");
...
frame main() ret int {
    ...
    # QUICK FIX: Import square from ./utils.adg
    return sq;
}
`,
      );
      ...
      console.log("  - Suggested fix: Import square from ./utils.adg");
    });
  });

  describe("Type Mismatch Errors", () => {
    it("creates test for type mismatch with cast suggestion", () => {
      const testFile = join(TEST_DIR, "ca_type_mismatch.adg");
      const code = `
frame main() ret int {
    local x: int = 42;
...
`;
    ...
    it("creates test for int to pointer cast", () => {
      const testFile = join(TEST_DIR, "ca_pointer_cast.adg");
      const code = `
frame main() ret int {
    local num: int = 42;
...
`;
  ...
  describe("Missing Return Statement", () => {
    it("creates test for missing return", () => {
      const testFile = join(TEST_DIR, "ca_missing_return.adg");
      const code = `
...
`;
  ...
  describe("Unused Variables", () => {
    it("creates test for unused variable", () => {
      const testFile = join(TEST_DIR, "ca_unused_var.adg");
      const code = `
frame main() ret int {
...
`;
    ...
    it("creates test for unused parameter", () => {
      const testFile = join(TEST_DIR, "ca_unused_param.adg");
      const code = `
...
`;
  ...
  describe("Missing Semicolons", () => {
    it("creates test for missing semicolon", () => {
      const testFile = join(TEST_DIR, "ca_missing_semicolon.adg");
      const code = `
frame main() ret int {
...
`;
  ...
  describe("Null Safety", () => {
    it("creates test for null pointer dereference", () => {
      const testFile = join(TEST_DIR, "ca_null_check.adg");
      const code = `
...
`;
    ...
    it("creates test for optional type access", () => {
      const testFile = join(TEST_DIR, "ca_optional_access.adg");
      const code = `
...
`;
  ...
  describe("Multiple Fixes for Same Error", () => {
    it("creates test with multiple possible fixes", () => {
      const file1 = join(TEST_DIR, "math1.adg");
      const file2 = join(TEST_DIR, "math2.adg");
      const mainFile = join(TEST_DIR, "ca_multiple_fixes.adg");
...
frame main() ret int {
    ...
    # 1. Import calculate from ./math1.adg
    # 2. Import calculate from ./math2.adg
...
`,
      ...
      console.log("    1. Import from ./math1.adg");
      console.log("    2. Import from ./math2.adg");
