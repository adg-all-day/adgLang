...
const TEST_DIR = join(tmpdir(), "adgLang-def-ref-test");
...
describe("Go to Definition Tests", () => {
  describe("Local Variables", () => {
    it("creates test for local variable definition", () => {
      const testFile = join(TEST_DIR, "def_local_var.adg");
      const code = `
...
`;
    ...
    it("creates test for function parameters", () => {
      const testFile = join(TEST_DIR, "def_parameter.adg");
      const code = `
...
`;
  ...
  describe("Functions", () => {
    it("creates test for function definition", () => {
      const testFile = join(TEST_DIR, "def_function.adg");
      const code = `
...
`;
    ...
    it("creates test for recursive function", () => {
      const testFile = join(TEST_DIR, "def_recursive.adg");
      const code = `
...
`;
  ...
  describe("Structs and Members", () => {
    it("creates test for struct definition", () => {
      const testFile = join(TEST_DIR, "def_struct.adg");
      const code = `
...
`;
    ...
    it("creates test for struct field definition", () => {
      const testFile = join(TEST_DIR, "def_field.adg");
      const code = `
struct Point {
...
`;
    ...
    it("creates test for struct method definition", () => {
      const testFile = join(TEST_DIR, "def_method.adg");
      const code = `
struct Calculator {
    value: int,
...
`;
  ...
  describe("Imports and Cross-File", () => {
    it("creates test for imported function", () => {
      const moduleFile = join(TEST_DIR, "math_utils.adg");
      const mainFile = join(TEST_DIR, "def_import_main.adg");

      fs.writeFileSync(
        moduleFile,
        `
export frame square(n: int) ret int {  # <-- LINE 2 in math_utils.adg: Definition
    return n * n;
}
...
`,
      ...
      fs.writeFileSync(
        mainFile,
        `
import [square, cube] from "./math_utils.adg";

frame main() ret int {
    local sq: int = square(5);  # <-- Cursor on square -> go to math_utils.adg line 2
    return sq;
}
`,
      );
      ...
      console.log(
        "  - Cursor on 'square' in main file → should jump to math_utils.adg:2",
      );
    });

    it("creates test for imported struct", () => {
      const typesFile = join(TEST_DIR, "types.adg");
      const mainFile = join(TEST_DIR, "def_import_struct.adg");

      fs.writeFileSync(
        typesFile,
        `
export struct Point {  # <-- LINE 2 in types.adg: Definition
    x: int,
    y: int,
}
...
`,
      ...
      fs.writeFileSync(
        mainFile,
        `
import [Point, Vector] from "./types.adg";

frame main() ret int {
    local p: Point = Point { x: 10, y: 20 };  # <-- Cursor on Point -> go to types.adg line 2
    return 0;
}
`,
      );
      ...
      console.log(
        "  - Cursor on 'Point' in main file → should jump to types.adg:2",
      );
    });
  });
  ...
  describe("Edge Cases", () => {
    it("creates test for shadowed variable", () => {
      const testFile = join(TEST_DIR, "def_shadowing.adg");
      const code = `
...
`;
    ...
    it("creates test for this pointer", () => {
      const testFile = join(TEST_DIR, "def_this.adg");
      const code = `
struct MyStruct {
    value: int,
...
describe("Find All References Tests", () => {
  describe("Local Variables - Scope Aware", () => {
    it("creates test for local variable references", () => {
      const testFile = join(TEST_DIR, "ref_local_var.adg");
      const code = `
frame foo() ret int {
...
`;
    ...
    it("creates test for parameter references", () => {
      const testFile = join(TEST_DIR, "ref_parameter.adg");
      const code = `
...
`;
  ...
  describe("Global Functions - All References", () => {
    it("creates test for function references", () => {
      const testFile = join(TEST_DIR, "ref_function.adg");
      const code = `
...
`;
    ...
    it("creates test for recursive function references", () => {
      const testFile = join(TEST_DIR, "ref_recursive.adg");
      const code = `
...
`;
  ...
  describe("Struct Fields - All References", () => {
    it("creates test for struct field references", () => {
      const testFile = join(TEST_DIR, "ref_struct_field.adg");
      const code = `
struct Point {
...
`;
    ...
    it("creates test for struct method references", () => {
      const testFile = join(TEST_DIR, "ref_struct_method.adg");
      const code = `
struct Calculator {
    value: int,
...
`;
  ...
  describe("Cross-File References", () => {
    it("creates test for imported function references", () => {
      const utilsFile = join(TEST_DIR, "ref_utils.adg");
      const mainFile = join(TEST_DIR, "ref_main.adg");
...
`,
      ...
      fs.writeFileSync(
        mainFile,
        `
import [square] from "./ref_utils.adg";  # <-- REFERENCE 2: Import
...
`,
      ...
      console.log(
        "  - ref_utils.adg:2 (declaration), ref_main.adg:2 (import), 5, 6 (calls)",
      );
    });
  });
  ...
  describe("Edge Cases", () => {
    it("creates test for variable vs field with same name", () => {
      const testFile = join(TEST_DIR, "ref_name_conflict.adg");
      const code = `
struct Data {
...
`;
    ...
    it("creates test for shadowing references", () => {
      const testFile = join(TEST_DIR, "ref_shadowing.adg");
      const code = `
frame testShadowing() ret int {
