...
# Bash completion script for adgLang CLI
# Installation:
#   1. Copy this file to /etc/bash_completion.d/adgLang or ~/.local/share/bash-completion/completions/adgLang
#   2. Or source it in your ~/.bashrc: source /path/to/adgLang-completion.bash
...
_adgLang_completion() {
        ...
        case "$command" in
            format)
                if [[ "$cur" == -* ]]; then
                    ...
                    # Complete with .adg files
                    COMPREPLY=( $(compgen -f -X '!*.adg' -- "\${cur}") )
                fi
                return 0
            ;;
            lint)
    ...
    if [[ "$cur" == -* ]]; then
        ...
        # Complete with commands and .adg files
        COMPREPLY=( $(compgen -W "\${commands}" -- "\${cur}") )
        COMPREPLY+=( $(compgen -f -X '!*.adg' -- "\${cur}") )
    fi
...
complete -F _adgLang_completion adgLang
`;
}
...
export function getZshCompletionScript(): string {
  return `#compdef adgLang
# Zsh completion script for adgLang CLI
# Installation:
#   1. Copy this file to a directory in your $fpath (e.g., /usr/local/share/zsh/site-functions/_adgLang)
...
_adgLang() {
    ...
    local -a commands
    commands=(
        'run:Compile and execute a ADGLANG program'
        'dev:Development mode with watch and auto-run'
        'build:Compile a ADGLANG program'
        'check:Type check ADGLANG files without generating code'
        'format:Format ADGLANG source files'
        'lint:Lint ADGLANG source files'
        'new:Create a new ADGLANG project with standard structure'
        'clean:Remove build artifacts and caches'
        'init:Initialize a new ADGLANG package'
        'pack:Package a ADGLANG project'
        'install:Install a ADGLANG package'
        'list:List installed ADGLANG packages'
        'uninstall:Uninstall a ADGLANG package'
    ...
    global_options=(
        '-e[Evaluate ADGLANG code passed as string]:code'
        '--eval[Evaluate ADGLANG code passed as string]:code'
        '--stdin[Read ADGLANG code from stdin]'
    ...
    case $state in
        command)
            _alternative \\
                ...
                'files:ADGLANG file:_files -g "*.adg"'
            ;;
        args)
            case $words[1] in
                format)
