 ...
 * Explicit compilation command for building ADGLANG programs
 */
 ...
 *
 * The `build` command is an explicit way to compile ADGLANG files.
 * It's more discoverable than the default action and makes intent clearer.
 *
 * Examples:
 *   adgLang build main.adg
 *   adgLang build main.adg -o output
 *   adgLang build main.adg -O3 --debug
 *   adgLang build main.adg --target x86_64-pc-windows-gnu
 *   adgLang build main.adg --emit llvm -v
 */
export function registerBuildCommand(program: Command): void {
  program
    .command("build")
    .argument("<file>", "ADGLANG file to compile")
    .description("Compile a ADGLANG program")
