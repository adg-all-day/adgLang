 ...
 * Executes scripts defined in adgLang.json
 */
...
export function registerRunScriptCommand(program: Command): void {
  program
    .command("run-script")
    .alias("rs")
    .argument("<script>", "Script name from adgLang.json")
    .argument("[args...]", "Arguments to pass to the script")
    .description("Run a script defined in adgLang.json")
    .action((scriptName: string, args: string[]) => {
      const manifestPath = path.join(process.cwd(), "adgLang.json");
      ...
      try {
        ...
        log.error("Failed to parse adgLang.json");
        process.exit(1);
      }

      if (!manifest.scripts || !manifest.scripts[scriptName]) {
        log.error(`Script '${scriptName}' not found in adgLang.json`);
        if (manifest.scripts) {
      ...
      const localBin = path.join(process.cwd(), "adgLang_modules", ".bin");
      const globalBin = path.join(os.homedir(), ".adg", "bin");
