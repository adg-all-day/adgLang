 ...
 * Monitors ADGLANG source files and triggers recompilation on changes
 */
...
/**
 * Watch a directory recursively and collect all .adg files
 */
function collectADGLANGFiles(dir: string): string[] {
  const files: string[] = [];
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      // Skip node_modules, .git, adgLang_modules, and other common ignore patterns
      if (
        entry.name === "node_modules" ||
        entry.name === ".git" ||
        entry.name === "adgLang_modules" ||
      ...
      if (entry.isDirectory()) {
        files.push(...collectADGLANGFiles(fullPath));
      } else if (entry.isFile() && entry.name.endsWith(".adg")) {
        files.push(fullPath);
      }
    }
  } catch (_e) {
...
export function watchMode(
  ...
  // Collect all .adg files in the directory tree
  const adgLangFiles = collectADGLANGFiles(watchDir);
  log.info(`Found ${adgLangFiles.length} ADGLANG files to watch\n`);
  ...
  // Watch each .adg file
  for (const file of adgLangFiles) {
