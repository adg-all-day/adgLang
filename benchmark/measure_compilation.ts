...
const TEMP_DIR = path.join(os.tmpdir(), "adgLang_bench");
...
function main() {
  ...
  const helloPath = path.join(__dirname, "../examples/hello-world/main.adg");
  measure("Hello World", helloPath);

  // 2. Medium file (Comprehensive Features)
  const compPath = path.join(
    __dirname,
    "../examples/comprehensive_features/main.adg",
  );
  ...
  const hugePath = path.join(TEMP_DIR, "huge.adg");
  ...
  const massivePath = path.join(TEMP_DIR, "massive.adg");
