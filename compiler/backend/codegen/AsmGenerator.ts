...
export abstract class AsmGenerator extends ExceptionGenerator {
  protected generateAsm(stmt: AST.AsmBlockStmt) {
    ...
    if (
        ...
        (match, eq, amp, name, constraint) => {
          if (this.locals.has(name)) {
            ...
            if (ptr && type) {
              ...
              return `__ADGLANG_ASM_OP_${key}__`;
            }
          }
          return match;
        },
      ...
      asmString = tempString.replace(/__ADGLANG_ASM_OP_(.+?)__/g, (match, key) => {
      ...
      if (stmt.clobbers && stmt.clobbers.length > 0) {
        ...
        const regRegex =
          /\b(%?)(r[abcd]x|e[abcd]x|[abcd]x|[abcd]l|[abcd]h|r[sd]i|e[sd]i|si|di|dil|sil|rbp|ebp|bp|adgLang|rsp|esp|sp|spl|r[89]|r1[0-5]|r[89][dwb]|r1[0-5][dwb]|xmm\d+|ymm\d+|zmm\d+)\b/gi;
        ...
        if (matches) {
          matches.forEach((m) => {
            ...
            else if (/^(rbp|ebp|bp|adgLang)$/.test(reg)) reg = "rbp";
