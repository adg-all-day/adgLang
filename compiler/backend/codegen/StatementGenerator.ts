...
export abstract class StatementGenerator extends AsmGenerator {
  ...
  protected generateReturn(stmt: AST.ReturnStmt) {
    ...
    if (!isMatchYield) {
      ...
      if (this.optimizationLevel < 2) {
        this.emit(`  call void @__adgLang_exit_stack_frame()`);
      }
  ...
  protected generateFunction(
    ...
    try {
      ...
      if (this.optimizationLevel < 2) {
        this.emit(`  call void @__adgLang_enter_stack_frame()`);
      }
      ...
      */
      ...
      if (name === "main") {
        this.emit(`  store i32 %argc, i32* @__adgLang_argc_value`);
        this.emit(`  store i8** %argv, i8*** @__adgLang_argv_value`);
      }
