 ...
 * type-checked ADGLANG AST nodes into LLVM IR text format.
 *
 * @example
 * ```typescript
 * const generator = new CodeGenerator({ target: "x86_64-linux-gnu" });
 * const llvmIR = generator.generate(typedAST, "main.adg");
 * ```
 *
...
export class CodeGenerator extends StatementGenerator {
  ...
  generate(program: AST.Program, filePath?: string): string {
    ...
    this.emitDeclaration(`@__adgLang_stack_depth = external global i32`);

    // Global argc/argv for Args library
    this.emitDeclaration(`@__adgLang_argc_value = external global i32`);
    this.emitDeclaration(`@__adgLang_argv_value = external global i8**`);
    ...
    if (!this.declaredFunctions.has("__adgLang_argc")) {
      this.emitDeclaration(`declare i32 @__adgLang_argc()`);
      this.declaredFunctions.add("__adgLang_argc");
    }
    if (!this.declaredFunctions.has("__adgLang_argv_get")) {
      this.emitDeclaration(`declare i8* @__adgLang_argv_get(i32)`);
      this.declaredFunctions.add("__adgLang_argv_get");
    }

    // Throw Helpers
    // These internal functions are safe to emit unconditionally (namespaced)
    this.emitDeclaration(`declare void @__adgLang_throw_stack_overflow()`);
    this.declaredFunctions.add("__adgLang_throw_stack_overflow");
    this.emitDeclaration(
      `declare void @__adgLang_throw_null_access(i8*, i8*, i32, i32)`,
    );
    this.declaredFunctions.add("__adgLang_throw_null_access");
    this.emitDeclaration(
      `declare void @__adgLang_throw_division_by_zero(i8*, i32, i32)`,
    );
    this.declaredFunctions.add("__adgLang_throw_division_by_zero");
    this.emitDeclaration(
      `declare void @__adgLang_throw_index_out_of_bounds(i32, i32, i8*, i32, i32)`,
    );
    this.declaredFunctions.add("__adgLang_throw_index_out_of_bounds");

    // Runtime Checks Declarations
    this.emitDeclaration(`declare void @__adgLang_enter_stack_frame()`);
    this.declaredFunctions.add("__adgLang_enter_stack_frame");

    this.emitDeclaration(`declare void @__adgLang_exit_stack_frame()`);
    this.declaredFunctions.add("__adgLang_exit_stack_frame");

    this.emitDeclaration(
      `declare void @__adgLang_check_null(i8*, i8*, i8*, i32, i32)`,
    );
    this.declaredFunctions.add("__adgLang_check_null");

    this.emitDeclaration("");

    // Helper: memory zero-check function used for 'struct == null' comparisons
    this.emitDeclaration("declare i1 @__adgLang_mem_is_zero(i8*, i64)");
