/**
 * ADGLANG Compiler Main Entry Point
 *
...
/**
 * Configuration options for the ADGLANG compiler.
 *
 * @example
 * ```typescript
...
export interface CompilerOptions {
  ...
  outputPath?: string;
  /** Type of output to generate: 'llvm' (IR), 'ast' (JSON), 'tokens' (JSON), 'formatted' (ADGLANG source) */
 ...
 * @example
 * ```typescript
 * import { Compiler } from 'adgLang/compiler';
 *
  ...
  private compileWithModuleResolution(): CompilationResult {
    try {
      ...
      for (const module of modules) {
        ...
        // Inject primitives into global scope if we just checked primitives.adg
        if (module.path.endsWith("primitives.adg")) {
      ...
      const isLinkingBpl = this.options.libraries?.includes("adgLang");
...
/** Source code formatter for ADGLANG files */
export { Formatter } from "./formatter/Formatter";
/** Tokenize ADGLANG source code using the PEG grammar */
export { lexWithGrammar } from "./frontend/GrammarLexer";
/** Parse ADGLANG tokens into an AST */
export { Parser } from "./frontend/Parser";
/** Type check and validate ADGLANG AST */
...
/** Lint ADGLANG code for style issues */
export { Linter } from "./linter/Linter";
/** Manage ADGLANG package dependencies */
