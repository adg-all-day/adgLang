...
export class Parser {
  ...
  public parse(
    ...
    // Implicitly import Error from std/errors.adg if not already in errors.adg
    const isErrorsBpl =
      this.filePath.endsWith("errors.adg") ||
      this.filePath.endsWith("errors.x") ||
      this.filePath.endsWith("intrinsics.adg");
    if (injectImplicitImports && !isErrorsBpl) {
      const errorImport: AST.ImportStmt = {
        kind: "Import",
        items: [{ name: "Error", isType: true, isWrapped: false }],
        source: "std/errors.adg",
        importAll: false,
        isImplicit: true,
        location: {
          file: this.filePath,
