...
function loadParser(): peggy.Parser {
  ...
  // Use ADGLANG_HOME to locate grammar file
  const grammarPath = resolveBplPath("grammar", "adgLang.peggy");

  if (!existsSync(grammarPath)) {
    const adgLangHome = getBplHome();
    throw new CompilerError(
      `Could not find adgLang.peggy grammar file.`,
      `Please ensure ADGLANG_HOME is set correctly or the grammar directory exists.\n` +
        `ADGLANG_HOME: ${adgLangHome}\n` +
        `Looking for: ${grammarPath}\n` +
        `You can set it with: export ADGLANG_HOME=/path/to/adgLang`,
      {
        file: grammarPath,
        startLine: 0,
        startColumn: 0,
