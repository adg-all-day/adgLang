/**
 * Operator overload resolution for the ADGLANG type checker
    ...
    for (const c of candidates) {
      ...
      if (ft.isVariadic) {
        if (decl.kind === "Extern") {
          ...
          // ADGLANG Variadic: (fixed..., variadic, count)
    ...
    for (const c of substitutedCandidates) {
      ...
      if (ft.isVariadic) {
        if (decl.kind === "FunctionDecl") {
          // ADGLANG Variadic: Check params to find the variadic boundary
