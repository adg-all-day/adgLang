...
export class ModuleResolver {
  ...
  private readonly SUPPORTED_EXTENSIONS = [".x", ".adg", ""];

  constructor(options: { stdLibPath?: string; searchPaths?: string[] } = {}) {
    // Use PathResolver to get the standard library path from ADGLANG_HOME
  ...
  private tryResolveWithExtensions(filePath: string): string | null {
    ...
    if (exists) {
      ...
      if (stat.isDirectory()) {
        ...
        for (const indexName of ["index.x", "index.adg"]) {
  ...
  resolveModules(entryFile: string): ModuleInfo[] {
    ...
    // Ensure primitives.adg is loaded
    const primitivesPath = this.normalizePath(
      path.join(this.stdLibPath, "primitives.adg"),
    );
