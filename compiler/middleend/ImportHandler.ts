/**
 * Import and module handling for the ADGLANG type checker
...
export class ImportHandler {
  ...
  private resolveImportPath(stmt: AST.ImportStmt): {
    ...
    if (this.ctx.skipImportResolution) {
      ...
      if (stmt.source === "std") {
        ...
        importPath = path.join(stdLibPath, "std.adg");
  ...
  public loadImplicitImports(): void {
    const stdLibPath = getLibPath();
    const primitivesPath = path.join(stdLibPath, "primitives.adg");
