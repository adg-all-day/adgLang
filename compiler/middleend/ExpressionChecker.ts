...
export function checkLiteral(
  ...
  if (expr.type === "number") {
    if (
      ...
      try {
        // Remove underscores which are allowed in ADGLANG but not in JS BigInt constructor
...
export function checkInterpolatedString(
  ...
  for (const part of expr.parts) {
    ...
    if (part.kind === "Literal" && part.type === "string") {
      ...
      // In ADGLANG `string` is alias for `*char` (or `*i8`).

      const partType = this.checkExpression(part);

      // Check for "string" (primitive string)
      // Note: In ADGLANG, 'string' is often an alias for '*char' or '*i8'
    ...
    if (operandType.kind === "BasicType") {
      if (operandType.isConst) {
        throw new CompilerError(
          ...
          "ADGLANG does not support pointers to constants yet. Taking the address would allow modification of the constant.",
          expr.location,
        );
      }
