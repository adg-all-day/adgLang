 ...
 *
 * Handles packaging, installation, and dependency management for ADGLANG projects.
 */
...
export class PackageManager {
  ...
  constructor(projectRoot?: string) {
    ...
    this.globalPackageDir = path.join(os.homedir(), ".adg", "packages");
    this.globalBinDir = path.join(os.homedir(), ".adg", "bin");

    // Local packages in project's node_modules equivalent
    const root = projectRoot || process.cwd();
    this.localPackageDir = path.join(root, "adgLang_modules");
  ...
  loadManifest(packageDir: string): PackageManifest {
    const manifestPath = path.join(packageDir, "adgLang.json");
    ...
    if (!fs.existsSync(manifestPath)) {
      throw new CompilerError(
        `No adgLang.json found in ${packageDir}`,
        "Run 'adgLang init' to create a new package.",
        location,
      );
    }
    ...
    try {
      ...
      if (!manifest.name) {
        throw new CompilerError(
          ...
          "Add a 'name' field to adgLang.json.",
          location,
        );
      }
      if (!manifest.version) {
        throw new CompilerError(
          ...
          "Add a 'version' field to adgLang.json (e.g., '0.1.0').",
          location,
        );
      }
      ...
      throw new CompilerError(
        ...
        "Check that adgLang.json is valid JSON.",
        location,
      );
    }
  }
  ...
  /**
   * Get all .adg files in directory recursively
   */
  private getAllBplFiles(dir: string): string[] {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        // Skip node_modules, adgLang_modules, .adg-cache, etc.
        if (
          item !== "node_modules" &&
          item !== "adgLang_modules" &&
          !item.startsWith(".")
        ) {
          files.push(...this.getAllBplFiles(fullPath));
        }
      } else if (item.endsWith(".adg")) {
        files.push(fullPath);
      }
    }
  ...
  pack(packageDir: string, outputDir?: string): string {
    ...
    if (files.length === 0) {
      throw new CompilerError(
        "No .adg files found in package",
        "Add some .adg files to the package directory.",
        {
          file: manifestPath,
          startLine: 1,
          startColumn: 1,
    ...
    const tempDir = path.join(os.tmpdir(), `adgLang-pack-${Date.now()}`);
    ...
    try {
      ...
      // Copy manifest
      fs.copyFileSync(manifestPath, path.join(packageRoot, "adgLang.json"));
  ...
  install(
    ...
    const tempDir = path.join(os.tmpdir(), `adgLang-install-${Date.now()}`);
    fs.mkdirSync(tempDir, { recursive: true });
  ...
  uninstall(
    ...
    const manifestPath = path.join(packagePath, "adgLang.json");
    if (!fs.existsSync(manifestPath)) {
      throw new CompilerError(
        `Invalid package directory: ${packagePath}`,
        "Directory exists but is not a valid package (missing adgLang.json).",
        {
          file: packagePath,
          startLine: 1,
          startColumn: 1,
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        try {
          ...
          // Skip invalid packages - they may lack adgLang.json or have invalid manifests
        }
      }
    }
  ...
  resolvePackage(packageName: string, projectRoot?: string): string | null {
    ...
    const checkPath = (baseDir: string): string | null => {
      ...
      if (parts.length > 0) {
        ...
        if (
          ...
          fs.existsSync(path.join(pkgPath, "adgLang.json"))
        ) {
          // Found package root
          if (parts.length === 1) {
            // Import "pkg" -> main
    ...
    // Check local
    let result = checkPath(path.join(root, "adgLang_modules"));
  ...
  private getPackageEntryPoint(packagePath: string): string | null {
    ...
    try {
      const manifest = this.loadManifest(packagePath);
      const entryPoint = manifest.main || "index.adg";
      const entryPath = path.join(packagePath, entryPoint);

      // Try with different extensions
      for (const ext of [".adg", ".x", ""]) {
        const fullPath =
          entryPath.endsWith(".adg") || entryPath.endsWith(".x")
            ? entryPath
            : entryPath + ext;
        if (fs.existsSync(fullPath)) {
          return fullPath;
  ...
  /**
   * Initialize a new ADGLANG project
   */
  init(dir: string, name?: string): void {
    const manifestPath = path.join(dir, "adgLang.json");
    if (fs.existsSync(manifestPath)) {
      throw new CompilerError(
        `adgLang.json already exists in ${dir}`,
        "Delete the existing adgLang.json if you want to re-initialize.",
        {
          file: manifestPath,
          startLine: 1,
          startColumn: 1,
