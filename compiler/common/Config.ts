 ...
 *
 * Provides a unified configuration interface for the ADGLANG compiler,
...
export const ENV_VARS = {
  ADGLANG_HOME: "ADGLANG_HOME",
  ADGLANG_CACHE_DIR: "ADGLANG_CACHE_DIR",
  ADGLANG_VERBOSE: "ADGLANG_VERBOSE",
  ADGLANG_DEBUG: "ADGLANG_DEBUG",
  NO_COLOR: "NO_COLOR",
} as const;
...
function getCacheDir(): string {
  ...
  const envCache = process.env[ENV_VARS.ADGLANG_CACHE_DIR];
  ...
  if (xdgCache) {
    return resolve(xdgCache, "adgLang");
  }

  // Default to ~/.cache/adgLang or platform equivalent
  const homeDir = process.env.HOME || process.env.USERPROFILE || "/tmp";
  return resolve(homeDir, ".cache", "adgLang");
}
...
function isVerbose(): boolean {
  return (
    process.env[ENV_VARS.ADGLANG_VERBOSE] === "1" ||
    process.env[ENV_VARS.ADGLANG_DEBUG] === "1"
  );
}
