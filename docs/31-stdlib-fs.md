...
The `FS` and `File` structs give you file system operations.

## Import

```adgLang
import [FS], [File] from "std/fs.adg";
```
...
Use this when you need more control over file operations:
...
frame main() {
    ...
    # Write to file
    if (FS.writeFile(path, "Hello, ADGLANG!\nLine 2\n")) {
