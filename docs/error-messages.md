...
The adgLang compiler now emits thorough, useful error messages with solid formatting, code context, and developer-friendly behavior. Every error shows precise location information in `file:row:column` format, along with code snippets and helpful hints.

## Features Implemented

### 1. **Precise Location Information**

Every error message shows the exact spot where the problem happens:

```
error[filename.adg:4:18]: Error description
```
...
When available, errors show the relevant code with nearby context:
...
Errors support severity categories:

- **Error**: Critical compilation failures
- **Warning**: Issues that do not stop compilation but may cause problems
...
Link related errors or earlier declarations:

```
related locations:
  main.adg:1:9: Function imported from here
  lib.adg:5:4: Original declaration here
```

Useful for:

- Pointing to conflicting declarations
- Linking back to the source of imported symbols
- Showing multiple related issues

### 6. **Error Summary**

When multiple errors happen, you get a summary:
...
Terminal output uses ANSI colors to make things easier to scan:
...
Colors respect the `NO_COLOR` environment variable so CI/CD stays compatible.
...
error[main.adg:5:10]: Type mismatch
     3 | fn main() {
     4 |   let x: i32 = 10;
     5 |   let y: string = x;
       |             ^^^^
...
The error formatting system covers all compiler error types:
...
- Supports debugging in the web interface
...
- Supports code navigation to the error location
...
- **Lazy Loading**: Source files are only read when an error is displayed
- **Caching**: Source lines are cached to avoid redundant reads
- **Graceful Fallback**: Still works without source files (shows location only)
- **Configurable**: Snippets can be disabled for better performance
