...
Tuples let you group multiple values, possibly with different types, into one compound value. They work well for returning multiple values from functions, temporary groupings, and pattern matching.
...
A tuple is an ordered, fixed-size collection of values where each element may have a different type.
...
- **Fixed size**: The number of elements is set at compile time
- **Heterogeneous**: Elements may have different types
- **Ordered**: You access elements by position
- **Value type**: Tuples are copied on assignment

## Creating Tuples

### Tuple Literals

Create tuples with parentheses and comma-separated values:
...
Destructuring lets you pull tuple elements out into separate variables:
...
Use `_` for elements you do not need:
...
One common use for tuples is returning multiple values from a function:
...
Functions can also take tuples as parameters:
...
Tuples work naturally with `match` expressions:
...
Tuples can hold other tuples too:
...
**Next:** See [Type Aliases](18-type-aliases.md) for a cleaner way to name complex types.
