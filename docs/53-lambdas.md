...
Lambda expressions, or anonymous functions, give you a compact way to define functions inline. They are especially useful with higher-order functions, callbacks, and functional-style code.
...
Basic lambda syntax uses pipes `|` around the parameter list:
...
Lambdas are often passed inline instead of being stored in a variable first:
...
adgLang provides two function pointer types:

### `Func<ReturnType>(ParamTypes...)`

Raw function pointer with C ABI compatibility. It cannot capture variables.
...
Fat pointer, meaning a struct with a function pointer and context. It can capture variables.
...
Lambdas can capture variables from the surrounding scope and form closures.

### Capture by Value (Default)

By default, variables are captured by **value** when the lambda is created:
...
If you want reference-style behavior and need to observe or change the original value, capture a pointer:
...
Structs are copied when you capture them by value:
...
Use `_` for parameters you do not need:
...
Functions can also return lambdas and closures:
...
Referencing a method on a struct instance creates a lambda with `this` captured:
