...
String interpolation lets you embed expressions right inside string literals. It is a cleaner way to build dynamic strings than doing manual concatenation everywhere.
...
Use backticks (`` ` ``) for an interpolated string. Put expressions inside `${...}`:
...
Any valid adgLang expression can go inside `${...}`:
...
The compiler lowers interpolated strings into `String` concatenation operations.
...
Different types are turned into strings in different ways:
...
If you want a literal `${` in the output, escape it with a backslash:
...
Standard escape sequences still work inside backtick strings:
...
For simple direct output where you do not need a `String` object:
...
If you are building large strings, a builder pattern or preallocation may be a better fit:
