...
This doc lays out a plan for refactoring how the adgLang compiler represents functions, structs, and polymorphism internally. The goal is straightforward: get **C-compatibility (FFI)**, **predictable memory layouts**, and **zero-cost abstractions**.

Right now, adgLang carries some hidden overhead:
...
The proposed fix is to split function types into **Raw Pointers** (`Func`) and **Closures** (`Lambda`), and move dynamic dispatch information from the **Object** to the **Reference** in the form of fat pointers and specs.

## 2. The Problem

### 2.1. The "Fat Pointer" Function

Currently, every `Func<Ret>(Args)` in adgLang is represented as:

```c
struct ADGLANGFunction {
...
- **FFI Incompatibility:** C expects `void (*)(...)` (8 bytes). Passing an adgLang function to `qsort` or `pthread_create` fails or requires complex wrappers.
...
We will split things into **Raw Functions** and **Closures**.
...
To support polymorphism, meaning calling different implementations through one common type, we reuse the existing `spec` keyword.
...
- **The VTable is carried by the variable `d`, not by the object `c`.**
- This is how Rust (`&dyn Trait`) and Go (`interface{}`) work.
- **Benefit:** You only pay the vtable cost when you actually use polymorphism.
...
```adgLang
# Cannot pass ADGLANG function directly because it's 16 bytes
extern qsort(...);
```
...
In this model, `struct` inheritance is only about **Data Layout** and **Code Reuse**. It does **not** imply polymorphism by default.
...
1. **Interoperability:** adgLang structs and functions become binary-compatible with C.
2. **Performance:**
