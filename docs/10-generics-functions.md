...
Generic functions let you write code that works across multiple types without giving up type safety. adgLang uses monomorphization, so the compiler emits a specialized version of a generic function for each concrete type that uses it.
...
Put generic parameters in angle brackets like `<T>` right after the function name:
...
The usual case is one type parameter:
...
When you call a generic function, the compiler can often infer the type arguments from the values you pass in:
...
You can also write the type arguments yourself when that makes things clearer or when inference is ambiguous:
...
adgLang supports type constraints with the `spec` keyword (specifications/interfaces):
...
Generics work cleanly with pointer types too:
...
When you use a generic function with a specific type, the compiler generates a specialized version for it:
...
**Next:** Move on to [Structs](11-structs.md) to build custom data types that work with generics.
