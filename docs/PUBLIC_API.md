# adgLang Compiler Public API Reference

This document covers the public API exported from the `compiler/` module for external use.

## Overview

The adgLang compiler is organized as a pipeline with separate phases:
...
This is the main compiler class, and it orchestrates the full compilation pipeline.
...
Tokenizes adgLang source code with the Peggy grammar.
...
| ------------ | ---------- | -------------------------------------------- |
| `sourceCode` | `string`   | The ADGLANG source code                          |
...
Contains all valid token types in the adgLang language.
...
Runs semantic analysis and type checking on the AST.
...
Formats adgLang source code into a consistent style.
...
Standard error class with source-location information.
...
// 1. Lex
const tokens = lexWithGrammar(source, "main.adg");

// 2. Parse
const parser = new Parser(source, "main.adg", tokens);
...
const llvmIR = generator.generate(ast, "main.adg");
```
