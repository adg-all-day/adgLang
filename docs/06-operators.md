...
This guide lists the operators available in adgLang, along with precedence, associativity, and the usual ways to use them.
...
adgLang follows C-like precedence rules. Operators with higher precedence bind first.
...
- `++x`: Increment, then return the new value
- `--x`: Decrement, then return the new value

### Type Promotion

adgLang requires explicit casts when arithmetic mixes types:
...
adgLang does not do implicit type promotion here. Cast operands to the same type before the operation.
...
adgLang requires explicit boolean expressions in control flow statements such as `if` and `loop`. Integers and pointers do not automatically convert to booleans.
...
Compound operators combine the operation and the assignment:
...
To access members through a struct pointer, dereference it first:
...
The `match` expression can test whether a value matches a specific type or pattern.
...
The `is` operator checks whether a struct pointer's runtime type matches, or derives from, a target type.
...
The `is` operator does a runtime vtable comparison and returns `true` when the actual type matches the target. See [Type Matching](56-type-matching.md) for more details.

### Safe Downcast (`as`)

The `as` operator tries a safe downcast and returns `nullptr` if the types do not match.
...
The `as` operator checks the vtable before casting. If the actual type does not match the target, it returns `nullptr` instead of doing an unsafe cast. See [Type Matching](56-type-matching.md) for more details.
...
- For arrays, it returns the total size, not the element count

## Offsetof Operator

The `offsetof` operator returns the byte offset of a field inside a struct declaration.
...
adgLang does **not** support operator overloading. Custom types cannot redefine operators.

If you want operator-like behavior for a custom type, use named methods instead:
...
1. **Use parentheses** for clarity, even when precedence already makes it valid
2. **Avoid deep ternary nesting** - use `if`/`else` once the condition gets complex
3. **Check pointers before dereferencing** to avoid crashes
4. **Be explicit with casts** - do not lean on implicit conversion
5. **Use compound assignment** for clarity and efficiency
6. **Prefer pre-increment** (`++i`) over post-increment (`i++`) when the value is not used
7. **Use sizeof** instead of hard-coding type sizes
8. **Watch for integer overflow** during arithmetic
9. **Be careful with precedence** - `a & b == c` means `a & (b == c)`, not `(a & b) == c`
