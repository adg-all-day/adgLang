...
Pointers are variables that hold memory addresses. You use them for dynamic memory, data structures, and efficient parameter passing. This guide walks through pointer usage in adgLang.
...
Use this to get a variable's address:
...
Use this to read or write the value at that address:
...
You can increment, decrement, and do arithmetic with pointers:
...
Pointer arithmetic automatically scales by the size of the type the pointer refers to:
...
Arrays and pointers are tightly connected in adgLang.
...
Function pointers let you store functions in variables and call them through those variables.
...
Pointers to pointers are handy when a function needs to modify a pointer.
...
6. **Use const for read-only pointers** (when available)
 ...
 ```adgLang
  # adgLang doesn't have const, but document intent
...
- Use indexes when they make the code clearer

8. **Document ownership**
- Make it obvious who allocates
- Make it obvious who frees
- Call it out in comments

## Next Steps

- [Memory Management](14-memory.md) - A deeper look at `malloc`, `free`, and memory management
- [Arrays](10-arrays-tuples.md) - More array techniques
