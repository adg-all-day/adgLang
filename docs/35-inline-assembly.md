...
adgLang lets you drop assembly straight into your programs. This is handy for hot paths, platform-specific CPU features, or talking to hardware directly. adgLang supports multiple assembly syntax flavors so you can use the style that fits your background or the job.
...
Use the `asm` block to place assembly instructions inside adgLang code:
...
Intel syntax uses the familiar `op dest, src` order. If you come from Windows or MASM-style tooling, this usually feels more natural.
...
AT&T syntax uses `op src, dest` order. You will see this style a lot in Unix/Linux tooling and GCC output.
...
If you need full control, you can write raw LLVM IR instructions directly:
...
adgLang includes solid variable interpolation so you can move cleanly between adgLang variables and assembly operands.
...
In raw LLVM mode, interpolation gives you pointers to variables:
...
Use explicit constraints when you need a specific register or operand class:
...
Clobbers tell the compiler which registers or resources your assembly changes so it can preserve them when needed.

### Syntax

Add the clobber list at the end of the assembly block:
...
For more involved cases, you can write LLVM IR directly:
...
**Next:** See [FFI (Foreign Function Interface)](36-ffi.md) to call C libraries from adgLang.
