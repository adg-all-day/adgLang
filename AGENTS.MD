---
applyTo: "**/*.adg, **/*.x, **/*.sh, **/*.ts, **/*.md"
---

# ADGLANG (Best Programming Language) Coding Assistant Instructions

You are working with ADGLANG, a statically-typed, compiled programming language that transpiles to LLVM IR. Follow these instructions strictly when writing code, creating examples, running tests, or modifying the compiler.
...
ADGLANG supports comprehensive pattern matching with the `match` expression:
...
import [Point] from "./geometry.adg";
import exec from "std/process.adg";
export frame myFunc() { ... }
```
...
- **Build Compiler**: `bun run build` -> Creates `adgLang` binary.
...
- **Format Examples**: `bun run format-examples` -> Formats ADGLANG files in `examples/`.
...
- Runs full compilation and execution of ADGLANG programs.
...
- **Files**: snake_case (`my_module.adg`).
...
- **Parser Issues**: If `grammar/adgLang.peggy` is modified, the parser is auto-generated at runtime (or cached). Ensure grammar logic is correct.
- **LLVM IR**: Use `--emit llvm` to inspect generated IR.
  ```bash
  bun index.ts examples/my_test/main.adg --emit llvm
  cat examples/my_test/main.ll
  ```
- **Segfaults**: Check `malloc`/`free` usage in ADGLANG code. Use `valgrind` on the generated binary if needed.
...
- `grammar/`: Peggy grammar file (`adgLang.peggy`).
